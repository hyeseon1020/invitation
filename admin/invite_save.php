...
if ($id) {
  $sql = "UPDATE tb_invitation SET invite_category=?, invite_theme=?, name1=?, name2=?, name3=?, name4=?, name5=?, name6=?, party_date=?, party_time=?, hall_name=?, hall_address=?, title=?, contents=? WHERE invite_id=?";
  ...
  $sql = "INSERT INTO tb_invitation (invite_code,invite_category,invite_theme,name1,name2,name1,name2,name1,name2,party_date,party_time,hall_name,hall_address,title,contents,created_id)
          VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param(
    "ssssssssssssssss",
